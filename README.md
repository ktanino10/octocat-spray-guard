...
|---|---|---|
| `octocat_spray_guard.stl` | STL | Standard 3D printing format (Git LFS) |
| `octocat_spray_guard.obj` | OBJ | General-purpose 3D format (Git LFS) |
| `octocat_spray_guard.3mf` | 3MF | Modern 3D printing format |
| `generate_spray_guard.py` | Python | Generation script |
...
3. **Spray paint** through the Octocat cutout
4. **Remove** the stencil → Octocat logo is transferred!
...
| ファイル | 形式 | 説明 |
|---|---|---|
| `octocat_spray_guard.stl` | STL | 3Dプリント標準フォーマット (Git LFS) |
| `octocat_spray_guard.obj` | OBJ | 汎用3Dフォーマット (Git LFS) |
| `octocat_spray_guard.3mf` | 3MF | 次世代3Dプリントフォーマット |
| `generate_spray_guard.py` | Python | 生成スクリプト |
